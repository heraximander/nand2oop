...
const CHIP_FN_TYPE_ERR: &str = "chip function must return type [ChipOutputInner;n] where n is a literal greater than 0";
...
pub fn chip(_: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let struct_name_str = &(name.chars().take(1).next().unwrap().to_uppercase().to_string() + &name[1..]);
